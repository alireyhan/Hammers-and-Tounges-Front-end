        ...
        <nav className="manager-side-drawer__nav">
          {navItems.map((item) => (
            <React.Fragment key={item.path}>
              {item.path === '/manager/buy' ? (
                <button
                  type="button"
                  className={`manager-side-drawer__link manager-side-drawer__link--btn ${showBuySubTabs ? 'active' : ''}`}
              ...
              {item.path === '/manager/buy' && showBuySubTabs && activeCategories.length > 0 && (
                <div className="manager-side-drawer__sub-tabs">
                  {activeCategories.map((cat) => {
                    const isCategoryActive = String(cat.id) === selectedCategoryId
                    return (
                      <Link
                        key={cat.id}
                        to={`/manager/buy?category=${cat.id}`}
                        className={`manager-side-drawer__sub-tab ${isCategoryActive ? 'active' : ''}`}
                      >
